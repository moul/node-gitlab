...
var Table, checkOptions, configFilePath, fs, getGitlabDataTypeMap, getTableHeadByData, gitlab, gitlabDircPath, makeTableByData, makeTableByDatas, makeTableByIssue, makeTableByProject, makeTableByUser, nconf, path, requireOrGetGitlab, tableHeadType;
...
makeTableByData = function(data, table_head) {
  var key, raw, table, value, _i, _len;
  if (table_head == null) {
  ...
  for (_i = 0, _len = table_head.length; _i < _len; _i++) {
    ...
    raw[data.id].push(value || "");
  }
  table.push(raw);
  return console.log(table.toString());
};

makeTableByDatas = function(datas, table_head) {
  var data, key, raw, table, value, _i, _j, _len, _len1;
  if (!datas.length) {
    return;
  }
  if (table_head == null) {

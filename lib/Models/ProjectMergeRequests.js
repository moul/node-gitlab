(function() {
  ...
  ProjectMergeRequests = (function(superClass) {
    ...
    ProjectMergeRequests.prototype.comment = function(projectId, mergerequestId, note, fn) {
      ...
      this.post("projects/" + (Utils.parseProjectId(projectId)) + "/merge_request/" + (parseInt(mergerequestId)) + "/comments", params, (function(_this) {
        return function(data) {
          if (fn) {
            return fn(data);
          }
      ...
      return {
        merge: (function(_this) {
          return function(projectId, mergerequestId, params, fun) {
            if (fun == null) {
              fun = null;
            }
            _this.debug("Projects::acceptMergeRequest()");
            params.id = Utils.parseProjectId(projectId);
            params.merge_request_id = parseInt(mergerequestId);
            return _this.put("projects/" + (Utils.parseProjectId(projectId)) + "/merge_request/" + (parseInt(mergerequestId)) + "/merge", params, function(data) {
              if (fn) {
                return fn(data);
              }
            });
